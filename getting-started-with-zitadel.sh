...
# Port na którym Caddy w Dockerze będzie nasłuchiwał ruchu od Twojego Nginxa
NETBIRD_DOCKER_PORT="10080"
# Twoja domena (zapisana na sztywno zgodnie z prośbą, by skrypt nie pytał)
NETBIRD_DOMAIN="netbird.cives.pl"
# ---------------------------------------
...
wait_api() {
    ...
    while true; do
      ...
      if [[ $counter -eq 45 ]]; then
        echo ""
        echo "Unable to connect to Zitadel for more than 45s. Check docker logs."
      fi
      echo -n " ."
      sleep 1
      counter=$((counter + 1))
...
init_zitadel() {
  ...
  # Mimo że docker ma http, używamy https w URLach bo host ma SSL
  ...
  # Tutaj curlujemy po localhost (bridge network exposed port) lub musimy to zrobic wewnatrz sieci dockera
  # W przypadku manual setup, skrypt dziala na hoscie. Host widzi port 10080 (zdefiniowany w NETBIRD_DOCKER_PORT)
  # Ale Caddy wewnatrz routuje po Host header. Musimy spoofować header w curlu.
  
  # ZMIANA: wait_api czeka na endpoint publiczny. Skoro Nginx moze jeszcze nie dzialac, 
  # powinnismy odpytac bezposrednio kontener Caddy na localhost
  
  echo -n "Waiting for Zitadel to become ready (checking localhost:$NETBIRD_DOCKER_PORT) "
  
  # Override wait logic for proxy setup
  set +e
  counter=1
  while true; do
      # Curl directly to exposed docker port, forcing Host header
      curl -s --fail --connect-timeout 1 -o /dev/null "http://127.0.0.1:$NETBIRD_DOCKER_PORT/auth/v1/users/me" \
        -H "Authorization: Bearer $PAT" \
      ...
      if [[ $counter -eq 60 ]]; then
        echo ""
        echo "Unable to connect to Zitadel via exposed port."
      fi
      echo -n " ."
      sleep 1
      counter=$((counter + 1))
  ...
  # Do funkcji wewnątrz używamy już pełnego publicznego URL, zakładając że użytkownik skonfigurował Nginxa 
  # LUB używamy triku z curlem resolve/proxy. 
  # Skrypt konfiguracyjny Zitadel używa INSTANCE_URL do API calli.
  # Musimy podmienic cURL-e w funkcjach? Nie, prościej jest zdefiniować INSTANCE_URL jako localhost 
  # ALE Zitadel zapisuje te URLe w konfiguracji (redirect URIs).
  
  # Rozwiązanie: Skrypt wykonuje zapytania do API NetBird/Zitadel. 
  # Skoro jesteśmy na hoscie, a Nginx może nie być gotowy, użyjmy 'connect-to' w curlu lub zmiennej.
  # Jednak najprościej: ZAKŁADAMY, że Nginx na hoście już działa i kieruje na port NETBIRD_DOCKER_PORT.
  # Jeśli nie, dodam alias do /etc/hosts lub poinstruuję usera.
  # W tym skrypcie zmodyfikuję funkcje by używały resolve jeśli trzeba, ale dla uproszczenia
  # założymy, że curlujemy po prostu publiczny URL (User musi miec DNS lub Nginx skonfigurowany).
  
  # Ale chwila - użytkownik dopiero stawia ten stack. Nginx może rzucać 502 dopóki kontenery nie wstaną.
  # Kontenery właśnie wstały. Więc INSTANCE_URL powinno działać przez Nginxa.
  ...
  # ZAWSZE HTTPS w tym scenariuszu, bo user ma Nginxa z SSL
  ...
  # CLI app works on localhost of the user machine so http is fine
...
initEnvironment() {
  CADDY_SECURE_DOMAIN=""
  # Zitadel configs for external proxy
  ZITADEL_EXTERNALSECURE="true"
  ZITADEL_TLS_MODE="external"
  ZITADEL_MASTERKEY="$(openssl rand -base64 32 | head -c 32)"
  
  # Setup for external SSL termination
  NETBIRD_PORT=443
  ...
  echo -e "\nDone!\n"
  echo "IMPORTANT: Configure your Host Nginx to proxy traffic to 127.0.0.1:$NETBIRD_DOCKER_PORT"
...
{
  debug
  # Listen strictly on port 80, no Auto HTTPS
  servers :80 {
    protocols h1 h2c h2 h3
    # Ufamy bramie dockera, żeby Caddy poprawnie wyciągnął {client_ip} z nagłówków Nginxa
    trusted_proxies static private_ranges
  }
}

:80 {
    # Konfiguracja globalna dla tej domeny
    # Zamiast powtarzać to w każdym bloku, zdefiniujmy nagłówki tutaj? 
    # Niestety w Caddy reverse_proxy lepiej to robić per backend.
...
services:
  ...
  caddy:
    ...
    networks: [ netbird ]
    ports:
      # Map host port $NETBIRD_DOCKER_PORT to container port 80 (No SSL inside)
      - '127.0.0.1:$NETBIRD_DOCKER_PORT:80'
    volumes:
